...
const getTransports = async (props = {}) => {
  let transports = await asap().transport.all(props);
  transports = await Promise.all(
    transports.map(async (e) => ({
      ...e,
      events: {
        TransportCreated: (
          await asap().company.getRawPastEvents('TransportCreated', {
...
export const fetchTransportInfo = createAsyncThunk(
  ...
  async ({ transportId }: { transportId: number }) => {
    let transport: ITransport = (await asap().transport.getById(
      transportId
    )) as ITransport;
    transport.events = {
      TransportCreated: (
    ...
    const batchInfo = await Promise.all(
      transport.batchIds.map(async (e) => await asap().batch.getById(e))
    );
    ...
    const events = await Promise.all(
      (await asap().transport.getStatusEvents(transportId)).map(
        async (event) => ({
          event,
          timestamp: (
