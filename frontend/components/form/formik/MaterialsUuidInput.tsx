...
const MaterialsUuidInput: React.FC<IMaterialsUuidInputProps> = (props) => {
  ...
  const getOptions = async () => {
    ...
    const {
      materialTokenId,
      uuid,
    } = await asap().material.getMaterialByUuid(value[0].id);
    console.log(uuid);
    if (uuid !== value[0].id) {
      setMaterialTokenInfo(null);
      setOptions([]);
    ...
    const uuids = await asap().material.getOwnedMaterialsUuidCodes(
      materialTokenId
    );
    const m = await asap().material.getById(materialTokenId);
