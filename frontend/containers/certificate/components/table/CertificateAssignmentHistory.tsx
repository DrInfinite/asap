...
import { CERTIFICATE_ASSIGNMENT_TYPE } from 'asap-library';
...
const CertificateAssignmentHistory: React.FC<ICertificateAssignmentHistoryProps> = ({
  ...
  useEffect(() => {
    (async () => {
      ...
      if (companyAddress) {
        fetchedHistory = (
          await asap().company.certificateAssignmentHistory({
            companyAddress,
            certificateCode,
          })
        )[certificateCode];
        ...
        fetchedHistory = (
          await asap().material.certificateAssignmentHistory({
            materialTokenId,
            certificateCode,
          })
        )[certificateCode];
      ...
      const historyWithBlocks = await Promise.all(
        fetchedHistory.map(async (e) => ({
          ...
          certificateInstance: await asap().material.getCertificateInstance(
