  ...
  describe('mint', () => {
    describe('raw material', () => {
      it('adds to the balance', async () => {
        ...
        const oldBalance = await asap.material.getBalance(
          materialTokenId
        );
        await (
        ...
        const newBalance = await asap.material.getBalance(
          materialTokenId
        );
    ...
    describe('material', () => {
      ...
      let batchId2: number;
      beforeAll(async () => {
        const result1 = await asap.material
          .create({
            name: 'product1',
            code: '123',
            amountIdentifier: 'kg',
        ...
        const createBatchResult1 = await asap.batch
          .create({
            materialsUuid: materialTokenId1Uuids,
            code: '1',
          })
          .send();
        const createBatchResult2 = await asap.batch
          .create({
      ...
      it('mints using multiple batches', async () => {
        ...
        const fetchedBatch1 = await asap.batch.getById(batchId1);
        const fetchedBatch2 = await asap.batch.getById(batchId2);
